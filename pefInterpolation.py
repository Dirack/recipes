...
#
# Site: https://wwww.geofisicando.com
# 
# Version 1.0
#
# Programmer:
...
def pefInterpolation(
	...
	for offsetGatherIndex in range(nhi):
		...
		Flow(pefCoeficients,[dataCube,zeroTraceGather,mask0],
		'''
		window n2=1 f2=%i |
		interleave axis=2 ${SOURCES[1]} |
		apef jump=2 a=%i,%i rect1=%i rect2=%i niter=%g verb=y
		maskin=${SOURCES[2]}
		''' % (offsetGatherIndex,a1,a2,rect1,rect2,totalPefIterations))

		# Interpolation
		Flow(interpolatedOffsetGather,[dataCube,zeroTraceGather,pefCoeficients,mask0],
		    '''
		window n2=1 f2=%i |
		interleave axis=2 ${SOURCES[1]} |
		miss4 exact=y filt=${SOURCES[2]} mask=${SOURCES[3]} niter=%g verb=y |
		put d2=%g
		''' % (offsetGatherIndex,totalInterpolationIterations,dm))
